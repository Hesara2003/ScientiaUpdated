...
export default function StudentManagement() {
  ...
  const fetchStudentsAndFees = async () => {
    ...
    try {
      ...
      // Ensure data is valid arrays to prevent mapping errors
      const validStudents = Array.isArray(studentsData) ? studentsData : [];
      const validFees = Array.isArray(feesData) ? feesData : [];
      
      console.log("Loaded students:", validStudents.length);
      console.log("Loaded fees:", validFees.length);
      console.log("Sample fee object:", validFees[0] || "No fees loaded");
      
      setFees(validFees);
      
      // If we don't have any students, avoid mapping and return early
      ...
      const studentsWithPayments = validStudents.map(student => {
        const studentId = student.studentId || student.id;        console.log(`Processing student: ${student.firstName} ${student.lastName}, ID: ${studentId}`);
        
        const studentFees = validFees.filter(fee => {
                    ...
                    <tr key={student.studentId || student.id} className="hover:bg-indigo-50/50 transition-all duration-200">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        ...
                        <div className="flex justify-end space-x-2">
                          <Link to={`/students/${student.studentId || student.id}/edit`} className="bg-indigo-50 p-2 rounded-lg text-indigo-600 hover:bg-indigo-100 transition-colors duration-150">
                            <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
