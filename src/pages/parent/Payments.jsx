...
import { getPurchasesByStudentId } from '../../services/tutePurchaseService';
...
export default function Payments() {
  ...
  const [parentId, setParentId] = useState('1'); // In a real app, get from auth
  ...
  // Fetch tutorial purchases for all children
  useEffect(() => {
    const fetchTutorialPurchases = async () => {
      try {
        ...
        // Fetch tutorial purchases for each student
        const allPurchases = [];
        
        for (const student of students) {
          const purchases = await getPurchasesByStudentId(student.id);
          
          // Get tutorial details for each purchase
          for (const purchase of purchases) {
            const tutorial = await getTuteById(purchase.tuteId);
            
            allPurchases.push({
              ...purchase,
              student: student,
              tutorial: tutorial,
              formattedDate: new Date(purchase.purchaseDate).toLocaleDateString('en-US', {
                year: 'numeric',
                month: 'short',
                day: 'numeric'
              })
        ...
        setTutorialPurchases(allPurchases);
      } catch (error) {
        console.error('Error fetching tutorial purchases:', error);
      } finally {
  ...
  const renderTabContent = () => {
    if (activeTab === 'payments') {
      ...
      return (
        ...
        >
          <motion.div variants={itemVariants}>
            <h2 className="text-xl font-bold text-gray-800 mb-4">Tutorial Purchases</h2>
            ...
            {loadingTutorials ? (
                      ...
                      {tutorialPurchases.map((purchase) => (
                        <tr key={purchase.id} className="hover:bg-gray-50">
                          <td className="px-6 py-4">
                            <div className="font-medium text-gray-900">{purchase.tutorial?.title || 'Unknown Tutorial'}</div>
                            <div className="text-sm text-gray-500">{purchase.tutorial?.category || 'Uncategorized'}</div>
                          </td>
                          <td className="px-6 py-4 whitespace-nowrap text-gray-700">
              ...
              <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-8 text-center">
                <p className="text-gray-600">No tutorial purchases found for your children.</p>
              </div>
            )}
          </motion.div>
        </motion.div>
